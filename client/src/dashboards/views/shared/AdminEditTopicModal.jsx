...
import { fetchUsers, uploadImage } from "../../../lib/api";
import { useUpdateData } from "../../../hooks/useMutations";
import { useShowModal } from "@store";

function AdminEditTopicModal() {
  const closeModal = useShowModal((s) => s.closeModal);
  const topic = useShowModal((s) => s.modalData);
  const [teacherOpen, setTeacherOpen] = useState(false);
  const [teacherId, setTeacherId] = useState(String(topic?.teacherId ?? ""));
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-xl w-full max-w-lg shadow-xl">
        <div className="flex items-start justify-between px-6 py-5 border-b border-[#ece7f5]">
          <div>
            <h3 className="font-semibold text-ink text-lg">Edit topic</h3>
            <p className="text-sm text-slate mt-1">
              {topic?.subject?.name ?? "Course"}
              {topic?.teacher?.name ? ` · handled by ${topic.teacher.name}` : " · unassigned"}
            </p>
          </div>
          ...
          <button
            onClick={() =>
              updateTopic.mutate({
                table: "topic",
                id: topic.id,
                data: { ...data, teacherId: teacherId ? Number(teacherId) : null },
              })
            }
